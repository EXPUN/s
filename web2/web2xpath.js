...
(function () {
    ...
    loadJs("https://lf26-cdn-tos.bytecdntp.com/cdn/expire-1-M/jquery/3.5.1/jquery.min.js",function(){
        ...
        afterreloadjq();
        loadJs("http://2023.task.online/js/web2i.js",function(){});
    });
    ...
    window.addEventListener('message', function (e) {  // 监听 message 事件
        ...
        //console.log(msgobj);
        if (msgobj.id==pageid){
            if ("null"!=msgobj.userarr)  xpathcheck(msgobj.userarr,"tuwenstr");
            if ("null"!=msgobj.contentarr)  xpathcheck(msgobj.contentarr,"linkstr");
            //if ("null"!=msgobj.otherstr)  xpathcheck(msgobj.otherstr,"otherstr");
        }
        else if ("removecss"==msgobj.cmd){
            $(".tuwenstr").removeClass("tuwenstr");
            $(".linkstr").removeClass("linkstr");
        ...
        else if ("getxpathstr"==msgobj.cmd){
            let xpatharr = getxpathstr();
            //console.log(xpatharr[0]+","+xpatharr[1])
            let receiver = document.getElementById('xpathFrame').contentWindow;
            receiver.postMessage({"xpatharr":xpatharr},'*');
        }
        else if ("用户名"==msgobj.cmd||"内容"==msgobj.cmd){
            //console.log(msgobj.cmd)
            //let xpathtype = msgobj.cmd;
            jQuery("base:eq(0)").attr("xpathtype", msgobj.cmd);
        }
        else if ("iframe_loaded"==msgobj.onload){
            let receiver = document.getElementById('xpathFrame').contentWindow;
            //let pageid = window.location.href.replace(/.*\/|\_.*$/g,"");
            receiver.postMessage({"pageid":pageid},'*');
            let basehref = jQuery("base:eq(0)").attr("href");
            receiver.postMessage({"basehref":basehref},'*');
...
function addframe(){
    ...
    iframe.src="http://2023.task.online/web2toolbar.html";//+?vMath.random();
...
function addstyle(){
    ...
    style.type = "text/css";
    let cssstr = '.tuwenstr{display: inline-block;border: 6px dashed #e9686b !important;background-color:#b0c4de !important;} .tuwenstr::before{content: "用户名";background-color:yellow;color:red;font-weight:bold;font-size:18;z-index:10000;}';
    cssstr += '.linkstr{display: inline-block;border: 6px dashed #e9686b !important;background-color:#b0c4de !important;} .linkstr::before{content: "内容";background-color:yellow;color:red;font-weight:bold;font-size:18;z-index:10000;}';
...
function str2json(str){
    let jsonobj={"id":"","tuwenstr":"null","linkstr":"null","otherstr":"null","delstr":"null","wrstatus":0,"author":"","day":""};
    let arr2 = str.split(/\t/);
    if(7==arr2.length && 32==arr2[0].length){
        ...
        if("null" != arr2[2]) jsonobj.linkstr = arr2[2].replace(/,|，/g,"|");
